# Keep the chart builders together so the dashboard uses one plotting style and one set of assumptions.

# Fix these palettes once so the same response and gender cues repeat across views.
LIKERT_PALETTE <- c(
...
# One base theme keeps the dashboard from drifting into chart-by-chart styling.
...
# Use the full response distribution because the mean score alone hides polarization and spread.
...
# Keep a mean-score chart too because it makes broad group differences easier to compare at a glance.
make_score_chart <- function(q_wide) {
  ...
  p <- ggplot2::ggplot(
    ...
    ggplot2::labs(
      subtitle = "Mean score (1 = Strongly Disagree, 5 = Strongly Agree)",
      y = "Score",
      fill = ""
    ) +
    chart_theme()
